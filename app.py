...
# Faqat FC2 qatlamdan iborat yakuniy klassifikator
...
# Bosh sahifa marshruti — Cookie ga qarab sahifa ko‘rsatish
@app.route("/")
def index():
    status = request.cookies.get('status', 'bot')  # default qiymat = bot
    if status == 'human':
        return render_template("human.html")
    return render_template("index.html")


# Bashorat marshruti — Bot yoki insonni aniqlash va Cookie o‘rnatish
...
def predict():
    ...
    # Agar kiruvchi ma’lumot dict bo‘lsa (JS ba'zida dict jo‘natishi mumkin)
    ...
    # Javobni tayyorlash va Cookie yangilash mantiği
    response = jsonify({"result": result})

    # Faqat kerak bo‘lsa Cookie ni o‘rnatish yoki yangilash
